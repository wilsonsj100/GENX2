---
title: "Project Porewater SO4/Cl"
...
###### Run information - PLEASE CHANGE
  Project = "Project"  
  Date_Run = "YYYY-MM-DD"  #Date that instrument was run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/Project_Samples_YYYYMM_Cl.txt"
  raw_file_name_so4 = "Raw Data/Project_Samples_YYYYMM_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/Project_Samples_Processed_Cl_SO4_YYYYMM.csv" 

###### Log Files - PLEASE CHECK 
#downloaded metadata csv of dilutions
  Raw_Metadata = "Raw Data/Project_PW_Dilutions.csv"
...
#Spike concentration calc 
  ...
  SampleVol <- 1500             # total volume of diluted sample in Dionex vial in uL
...
####Edit this to match specific project####
...
#   rename("Cl_SO4_ID" = "Impoundment.Location...Plot...Replicate") %>%
#   mutate(Cl_SO4_ID = toupper(Cl_SO4_ID)) 

# colnames(metadata)

#Take out the columns and rows that are not relevant
dionex_metadata <- metadata %>%
  # select(Cl_SO4_ID, Site, Subsite, Subsite.Abbreviation, Inpoundment.Location, Plot, 
  #        Replicate, Sampling.Date, Time.Sample.Taken..CT., Dilution, Actual.Volume..mL., Refractometer.reading..ppt., Notes ) %>%        
...
#Separate sample ID's 
all_dat <- all_dat %>%
  mutate(sample_ID = sub("^[^_]+_", "", sample_ID))

#Separate dup/spk namings
all_dat <- all_dat %>%
  mutate(sample_ID_long = sample_ID) %>%
  separate(sample_ID_long, into = c("sample_ID_short", "test"), sep = "_")

#merge dilution with sample run data 
merged_data <- all_dat %>%
  left_join(dionex_metadata, by = c("sample_ID_short" = "Cl_SO4_ID"))
...
spks <- merged_data %>%  
  select(c(sample_ID, SO4_Conc_mM, SO4_ppm, Dilution)) %>%
  filter(str_detect(sample_ID, "spk"))      #have to change this to match data
# head(spks)

#create a new dataframe and remove dups from sample dataframe 
dat_spks <- merged_data %>%  
  filter(!str_detect(sample_ID, "spk")) %>%  
  select(c(sample_ID, SO4_Conc_mM, SO4_ppm))
...
#mmol/L * L = mmol
#Want actual amount so divide by the dilution used by the Dionex to calculate the concentration 
...
#Dilute samples 
merged_data$Cl_ppm_dil <- merged_data$Cl_ppm/merged_data$Dilution
merged_data$SO4_ppm_dil <- merged_data$SO4_ppm/merged_data$Dilution

#Remove standards
dat_flagged <- merged_data %>%  
...
####Edit this if there are IDs with distinctive plots to visualize####
...
viz_cl_plot <- ggplot(data_plotting, aes(x = row_num, y = Cl_ppm, fill = Subsite)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single")) +
  facet_grid(~ Subsite, scales="free_x") +
  theme_classic() +
  theme(axis.text.x = element_blank()) +
...
viz_so4_plot <-  ggplot(data_plotting, aes(x = row_num, y = SO4_ppm, fill = Subsite)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single")) +
  facet_grid(~ Subsite, scales="free_x") +
  theme_classic() +
  theme(axis.text.x = element_blank()) +
...
final_data <- df_all_clean %>% 
  mutate(
    Project = Project,   # change this in setup chunk, new column with same value on every row
...
final_data <- final_data %>%
  rename(
    Time.Sample.Taken = Time.Sample.Taken..CT.,
    Actual.Volume.mL = Actual.Volume..mL., 
    Refractometer.reading.ppt = Refractometer.reading..ppt., 
    Field_Notes = Notes, 
    Sample_ID = sample_ID
    # add more rename pairs as needed
  ) %>%
  select(
         Project, Site, Subsite, Subsite.Abbreviation, Inpoundment.Location, Plot, Replicate, 
         Sample_ID, Sampling.Date, Time.Sample.Taken, Actual.Volume.mL, Refractometer.reading.ppt, 
