---
title: "GENX2 Porewater SO4/Cl"
author: "2025 Samples Run 1"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: true
...
lapply(c(
  ...
  "matrixStats", "gridExtra", "grid", "tidyverse", "knitr", "readr"), 
...
###### Run information - PLEASE CHANGE
  ...
  run_notes = "Sample 7_GX2_50151 was above the SO4 detection limit (22 ppm vs the 20 ppm detection limit)
...
metadata <- read.csv(Raw_Metadata)
...
#subset the log file by the current year
log_year <- log %>%
  filter(str_detect(run_date, Year_run))   
  
#plot the current slopes with the previous slopes 
Slopes_chk <- ggplot(log_year, aes(run_date, slope, col=curve)) +
  geom_point(size=4) + 
  geom_line() + 
...
spks <- merged_data %>%  
  ...
  filter(str_detect(sample_ID, "SPIKE"))      #have to change this to match data
# head(spks)

#create a new dataframe and remove dups from sample dataframe 
dat_spks <- merged_data %>%  
  filter(!str_detect(sample_ID, "SPIKE")) %>%  
  select(c(sample_ID, SO4_Conc_mM, SO4_ppm))

#remove the dup from these IDs so we will have duplicate sample names
spks <- spks %>%
  mutate(sample_ID = gsub("_SPIKE", "", as.character(sample_ID))) %>%
  rename(
    sample_ID = sample_ID,
...
```{r Visualize Data, include = FALSE, echo=FALSE, warning=FALSE}
# 
